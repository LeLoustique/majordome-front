...
export default function Chat() {
  ...
  const handleSend = async () => {
    ...
    try {
      // Appel à l'API avec token + refresh token
      const data = await sendMessageToChatController(message, refreshAuthToken, token);

      // Ici on suppose que le backend renvoie un objet avec { response: "texte" }
      // Si ton backend renvoie juste un string, adapte ici : data.response => data
      const replyText = data.response || data;

      const replyTime = new Date().toLocaleTimeString();
      setMessages((prev) => [...prev, { from: "bot", text: replyText, time: replyTime }]);
    } catch (error) {
      const replyTime = new Date().toLocaleTimeString();
      setMessages((prev) => [
        ...prev,
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-4 bg-gray-100">
        {messages.map((msg, idx) => (
          ...
          >
            <div>{msg.text}</div>
            <div className="absolute bottom-1 right-2 text-xs text-gray-600">{msg.time}</div>
          </div>
        ))}
